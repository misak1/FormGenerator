...
function modify_submit(memberCount){
	var flag = false;
	for(i=0; i<memberCount; i++){
		obj = document.getElementById('checkmember_id'+i);
		if(obj.checked){
			flag = true;
			break;
		}
	...
	if(!flag){
		alert('編集するメンバーが選択されていません。');
	} else {
		document.frm2.pagemode.value="modify";
		document.frm2.action="edit8_edit.php";
		document.frm2.submit();
	}

	//document.frm2.frm2_action.value="modify";
	//document.frm2.submit();
}
function delete_submit(memberCount){
	var flag = false;
	for(i=0; i<memberCount; i++){
		obj = document.getElementById('checkmember_id'+i);
		if(obj.checked){
