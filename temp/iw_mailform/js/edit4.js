...
function modify_submit(replycategorymemberCount){
	var flag = false;
	for(i=0; i<replycategorymemberCount; i++){
		obj = document.getElementById('checkreplycategorymember_id'+i);
		if(obj.checked){
			flag = true;
			break;
		}
	...
	if(!flag){
		alert('編集する紐付けが選択されていません。');
	} else {
		document.frm2.pagemode.value="modify";
		document.frm2.action="edit4_edit.php";
		document.frm2.submit();
	}
}
function delete_submit(replycategorymemberCount){
	var flag = false;
	for(i=0; i<replycategorymemberCount; i++){
		obj = document.getElementById('checkreplycategorymember_id'+i);
		if(obj.checked){
	...
	if(!flag){
		alert('削除する紐付けが選択されていません。');
	} else {
		document.frm2.pagemode.value="delete";
		document.frm2.submit();
	}
}