...
function modify_submit(replyCount){
	var flag = false;
	for(i=0; i<replyCount; i++){
		obj = document.getElementById('checkreply_id'+i);
		if(obj.checked){
			flag = true;
			break;
		}
	...
	if(!flag){
		alert('編集するメールアドレスが選択されていません。');
	} else {
		document.frm2.pagemode.value="modify";
		document.frm2.action="edit10_edit.php";
		document.frm2.submit();
	}
}
function delete_submit(replyCount){
	var flag = false;
	for(i=0; i<replyCount; i++){
		obj = document.getElementById('checkreply_id'+i);
		if(obj.checked){
