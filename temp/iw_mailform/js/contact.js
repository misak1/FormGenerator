...
function validateForm(){
	// プライバシーポリシーのチェック
	if(elm('chkpolicy').checked || elm('chkpolicy').value == 1){
		// OK
	} else {
		return false;
	}
	...
	if(parseInt(elm('selectcategory').value) == 0){
		canpost = false;
		ccategory = "errorCol";
		err += errorMessage("EMPTY_CATEGORY");
	}
	...
	if(isNullOrInvalid(elm('txtusername').value, spaces)){
		canpost = false;
		cuser = "errorCol";
		err += errorMessage("EMPTY_FULLNAME");
	}
	...
	if(isNullOrInvalid(elm('txtemail').value, spaces)){
		canpost = false;
		cemail = "errorCol";
		err += errorMessage("EMPTY_E-MAIL");
	}
	else if(! isValidMailAddress(elm('txtemail').value)){
		canpost = false;
		cemail = "errorCol";
		err += errorMessage("REJECT_E-MAIL");
	}
	...
	if(isNullOrInvalid(elm('tacontent').value, spaces)){
		canpost = false;
		ccontent = "errorCol";
		err += errorMessage("EMPTY_INQUIRY");
	}
...
function buttonView(){
	...
	if(elm('chkpolicy').checked || elm('chkpolicy').value == 1){
		elm('btnconfirm').src = '/iw_mailform/images/btn_confirm.gif';
		elm('btnconfirm').disabled = false;
	} else {
		elm('btnconfirm').src = '/iw_mailform/images/btn_confirm_disable.gif';
		elm('btnconfirm').disabled = true;
	}
}
...
function errorMessage(key){
	...
	var dict ={
		"EMPTY_CATEGORY":
			{
			"ja":"カテゴリを選択してください。<br />\r\n",
			"en":"Please select a category.<br />\r\n"
			}
		,
		"EMPTY_FULLNAME":
			{
			"ja":"お名前は必須項目です。<br />\r\n",
			"en":"Please fill out your NAME.<br />\r\n"
			}
		,
		"EMPTY_E-MAIL":
			{
			"ja":"メールアドレスは必須項目です。<br />\r\n",
			"en":"Please fill out E-MAIL.<br />\r\n"
			}
		,
		"REJECT_E-MAIL":
			{
			"ja":"メールアドレスをご確認ください。<br />\r\n",
			"en":"Please check the E-MAIL address.<br />\r\n"
			}
		,
		"EMPTY_INQUIRY":
			{
			"ja":"お問い合わせ内容は必須項目です。<br />\r\n",
			"en":"Please fill out INQUIRY.<br />\r\n"
			}
	};
	var str = dict[key][lang];
	return str;
}