...
function modify_submit(categoryCount){
	var flag = false;
	for(i=0; i<categoryCount; i++){
		obj = document.getElementById('checkcategory_id'+i);
		if(obj.checked){
			flag = true;
			break;
		}
	...
	if(!flag){
		alert('編集するカテゴリーが選択されていません。');
	} else {
		document.frm2.pagemode.value="modify";
		document.frm2.action="edit7_edit.php";
		document.frm2.submit();
	}
}
function delete_submit(categoryCount){
	var flag = false;
	for(i=0; i<categoryCount; i++){
		obj = document.getElementById('checkcategory_id'+i);
		if(obj.checked){
